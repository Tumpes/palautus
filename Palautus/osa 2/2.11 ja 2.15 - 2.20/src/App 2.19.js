  ...
  return (
    <ul>
      {props.filteredNames.map((name, id) => (
        <props.Nimi name={name} key={name.name} id={id} setNotification={props.setNotification} setNotificationType={props.setNotificationType} />
      ))}
    </ul>
  );
};
...
const App = () => {
  ...
  const addName = (event) => {
    ...
    if (persons.filter((e) => e.name === newName).length !== 0) {
      ...
      setNotification(`henkilön ${newName} numero vaihdettu numeroon ${newNumber}`)
  ...
  return (
    <div>
      ...
      />
      <h2>Numbers</h2>
      <Persons filteredNames={filteredNames} Nimi={Nimi} setNotification={setNotification} setNotificationType={setNotificationType} />
    </div>
  );
};
